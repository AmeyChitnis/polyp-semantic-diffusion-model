...
def load_data(
    *,
    ...
    elif dataset_mode == 'custom_dataset':
        all_files = _list_image_files_recursively(os.path.join(data_dir, 'images', 'training' if is_train else 'validation'))
        classes = _list_image_files_recursively(os.path.join(data_dir, 'annotations', 'training' if is_train else 'validation'))
        instances = None  # Assuming no instances in the custom dataset
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.dataset_mode == 'custom_dataset':
        # Handling of resizing or cropping for your custom dataset. Adjust as needed.
            #if self.is_train:
             #   if self.random_crop:
           ...
           # else:
            ...
            # Normalizing binary mask values to 0 and 1
            arr_class = (arr_class > 128).astype(np.uint8)  # Convert values >128 to 1, otherwise 0
